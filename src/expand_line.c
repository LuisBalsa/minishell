...
/*   Updated: 2023/08/10 03:36:56 by luide-so         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	point_to_expand(int point, char *tmp, t_shell *shell)
{
	char	*key;
	int		len;
	...
	if (*tmp == '~')
	{
		if (!tmp[1] || ft_strchr(" /", tmp[1]))
			expand(get_env("HOME", shell), point, point + 1, shell);
		else if (tmp[1] == '+' && (!tmp[2] || ft_strchr(" /", tmp[2])))
			expand(get_env("PWD", shell), point, point + 2, shell);
		else if (tmp[1] == '-' && (!tmp[2] || ft_strchr(" /", tmp[2])))
			expand(get_env("OLDPWD", shell), point, point + 2, shell);
	}
	else if (*tmp == '$' && tmp[1] == '?')
		expand(ft_itoa(g_exit), point, point + 2, shell);
...
static void	expand_tilde(t_shell *shell)
{
	...
	while (*tmp)
	{
		...
		{
			point_to_expand(tmp - shell->line, tmp, shell);
			tmp = shell->line;
		}
		tmp++;
	}
